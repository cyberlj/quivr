...
| D02 | Benchmark statistics | Need one stable latency metric and aggregation rule for scoring. | decided | Implement the benchmark runner output contract. | Owned by `benchmark-spec.md`. Primary gain metric is `full_session_p50_ms`, guard metric is `session_p90_ms`, and real-provider E2E runs as shadow observation. | 2026-04-10 |
...
| D07 | Benchmark noise handling | Noisy runs can create false keep or false reset decisions. | decided | Implement rerun and invalid-result handling in the benchmark runner. | Owned by `OPERATING_MODEL.md` and `benchmark-spec.md`. Improvement under 5 percent is inconclusive, opposite reruns are noisy, p90 guard is enforced, and shadow E2E drift does not replace the primary gain baseline. | 2026-04-10 |
...
| D19 | Observation layer model | The harness needs explicit record, summary, and alert layers so a human can observe long-running execution. | decided | Implement the recorder and observer flow later. | Owned by `observation-model.md`. Recorder, Observer, and Dashboard roles are now defined. | 2026-04-10 |
| D20 | Event and telemetry schemas | Logs must be structured or they will not support replay, summaries, or dashboards. | decided | Implement emitters for event, tool, and API logs later. | Owned by `event-schema.md`. JSONL schemas are now defined. | 2026-04-10 |
| D21 | Observation write ownership | Observation and control layers need one owner per fact and an anti-recursion rule. | decided | Implement recorder ownership rules later. | Owned by `observation-model.md`, `event-schema.md`, `agent-contract.md`, and `AGENTS.md`. Recorder no longer co-owns runtime state and observation writes do not recursively log themselves. | 2026-04-10 |
| D22 | Single waiting-state semantics | Waiting must be expressed through one state machine field, not parallel booleans. | decided | Implement state transitions with `loop_status` as the source of truth. | Owned by `runtime-state.md` and `decision-log.md`. | 2026-04-10 |
| D23 | Candidate lifecycle rules | Candidate validity needs explicit status and transition rules to support pool-empty and exhaustion checks. | decided | Implement lifecycle updates in the controller later. | Owned by `OPERATING_MODEL.md`, `planning-model.md`, and `decision-log.md`. | 2026-04-10 |
